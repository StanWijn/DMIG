...
names(p.ed) <- names(c.ed) <- names(c.ld) <- c("RP", "PC", "HC")  # names for the elements of the three vectors
...
### BRING THIS FURTHER UP (AS WELL AS THE INPUT DATA FRAME)
dec_tree <- function(params){
  with(
    as.list(params),
    {
      ...
      LE <- c(e.RP, e.PC, e.HC)  # vector of life expectancies
      C  <- c(c.RP, c.PC, c.HC)  # vector of total costs
      nmb <- LE * wtp - C        # calculate nmb
      names(LE) <- paste("LE", c("RP", "PC", "HC"), sep = "_")   # names for the elements of the  LE vector
      names(C) <- paste("C", c("RP", "PC", "HC"), sep = "_")     # names for the elements of the  C  vector
      names(nmb) <- paste("NMB", c("RP", "PC", "HC"), sep = "_") # names for the elements of the nmb vector
...
legend("bottomright", Strategies, col = 1:3, lty = c(1, 1, 1), bty = "n", cex = 0.7)

### the ggplot way
source("Functions/owsa_diagram_code.R")
paramName <- "p.PCed"
...
### the ggplot way
source("Functions/twsa_diagram_code.R")
outcomeName <- "Net Monetary Benefit"
...
## Parameter names
paramNames <- c("p.PCed", "c.PCed", "c.PCld", "le.ld")
